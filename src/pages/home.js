    ...
    <Container>
      <InputGroup className="mb-3 mt-3">
        <Form.Control placeholder="Rechercher..." onChange={(event) => 
            setFilteredPosts(allPosts.current.filter(post => post.body.replace(/(\r\n|\n|\r)/gm, " ").includes(event.target.value)))} />
      </InputGroup>
      <h6>
        <Badge bg="secondary">{filteredPosts.length} </Badge> posts sur <Badge bg="secondary">{allPosts.current.length} </Badge>
      </h6>
